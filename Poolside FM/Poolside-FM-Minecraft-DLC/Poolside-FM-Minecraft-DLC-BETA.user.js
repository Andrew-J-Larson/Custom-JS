...
var youtubeLoader = null; // gets set when loaded
...
var fixGuiMC = function(webWindow, webIframe) {
    ...
    webWindow.style = iPreStyle + "width: " + (webWindow.clientWidth - 1) + "px; height: " + (webWindow.clientHeight - 1) + "px;";
    setTimeout(function() {webWindow.style = iStyle;}, 500);
}
...
var replaceIframe = function(webInnerContent, webWindow) {
    ...
    webIframe.focus(); // fixes WebGL not loading error on some websites that start up

    // fix Minecraft GUI after every iframe reload
    webIframe.onload = function() {fixGuiMC(webWindow, webIframe)}
}
...
var convertToMinecraft = function() {
    ...
    // style the window
    webInnerContent.style.background = "white url('" + APP_SPLASH + "') no-repeat fixed center;";
    ...
    reloadIcon.onclick = function() {replaceIframe(webInnerContent, webWindow)}
...
var waitForIframe = setInterval(function() {
    ...
    if (exists(webIframe)) clearInterval(waitForIframe);
}, LOOP_TIME);
var loadMain = setInterval(function() {
    if (!exists(webIframe)) return; // wait to run this until iframe is loaded

    youtubeLoader = document.querySelector('.youtube-loader');
    var ytLoaded = exists(youtubeLoader);
    if (ytLoaded) {
        loadedYT = true;
    }
    if (!ytLoaded && loadedYT && !isHidden(webIframe)) {
